...
func (p *PacketFragment) Marshall() []byte {
	...
	buf.Write(common.Uint32ToBytes(p.Id))
	buf.Write(common.Uint16ToBytes(p.Count))
	if p.Count > 1 {
		buf.Write(common.Uint16ToBytes(p.FragSeq))
	}
	buf.Write(p.Data)
	return buf.Bytes()
}
...
func (p *PacketFragment) Unmarshal(m []byte) error {
	...
	p.Id = common.BytesToUint32(m[PHEAD : PHEAD+4])
	p.Count = common.BytesToUint16(m[PHEAD+4 : PHEAD+6])
	var data []byte
	if p.Count > 1 {
		p.FragSeq = common.BytesToUint16(m[PHEAD+6 : PHEAD+8])
		data = m[PHEAD+8:]
	} else {
		data = m[PHEAD+6:]
	}
